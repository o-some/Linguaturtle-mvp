...
if(params.get('qa')==='1'){
  ...
  const click=async selector=>{const el=document.querySelector(selector);if(!el)return false;el.click();await sleep(180);return true};
  ...
  const panel=()=>{
    ...
    document.querySelector('#qaClose')?.addEventListener('click',()=>{restore();location.href=location.pathname+'?build=v3.0.2-qa';});
  };
  async function run(){
    await sleep(1600);
    ...
    if(firstPlace){firstPlace.click();await sleep(180)}
  ...
  run().catch(err=>{assert('QA-Lauf ohne Ausnahme',false,String(err));restore();panel();});
}
