...
export function registerAdvancedRoutes(router,{top,nav,renderComplete}){
 ...
 router.register('speed',()=>{if(!speed.items.length)startSpeed();return `<div class="v3-shell page speed-bg">${top('world')}<section class="page-title speed-title"><span class="eyebrow">${tr('ZEIT-ABENTEUER','AVENTURA CONTRARRELOJ')}</span><h1>${tr('Goldene Minute','Minuto dorado')}</h1><p>${tr('Übersetze so viele Wörter wie möglich.','Traduce tantas palabras como puedas.')}</p></section><section class="speed-status"><div><small>${tr('ZEIT','TIEMPO')}</small><strong id="speedTime">${speed.time}</strong></div><div><small>COMBO</small><strong>${speed.combo}×</strong></div><div><small>${tr('PUNKTE','PUNTOS')}</small><strong>${speed.score}</strong></div></section><section class="speed-question"><span>${speed.current.emoji}</span><small>${tr('ÜBERSETZE','TRADUCE')}</small><h2>${speed.current[getState().language]}</h2></section><section class="speed-options">${speed.options.map(w=>`<button data-action="speed-answer" data-answer="${w.id}">${w[target()]}</button>`).join('')}</section></div>`});
}
