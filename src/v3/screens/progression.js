import {
  ...
  creditGameplayShells, MILESTONE_LEVELS
} from '../core/index.js?build=cinematic-worlds-1';
import { assets } from '../../config/assets.js?build=cinematic-worlds-1';
import { renderProfileLanguageSection } from './language-passport.js?build=cefr-1';
import {
...
export function renderProfile({top,nav,progressCard}){
 const s=getState(),lv=levelFromXp(s.progress.xp),stars=totalMasterStars(s);
 const pending=milestones.filter(level=>!s.inventory.claimedMilestones.includes(level));
 const visible=pending.slice(0,3),history=milestones.filter(level=>!visible.includes(level));
 const milestoneCard=level=>{
  const reached=lv>=level,claimed=s.inventory.claimedMilestones.includes(level);
  const focused=Number(s.session.focusMilestone)===level,claimReady=reached&&!claimed;
  return `<article class="milestone-card ${reached?'reached':''} ${claimReady?'claim-ready':''} ${focused?'milestone-focus':''}" data-milestone="${level}"><img class="milestone-chest" src="${milestoneChest(level)}" alt=""><div><small>${tr('INSELSTUFE','NIVEL DE ISLA','ΕΠΙΠΕΔΟ ΝΗΣΙΟΥ','ISLAND LEVEL')} ${level}</small><strong>${tr('Meilenstein-Belohnung','Recompensa de nivel','Ανταμοιβή επιπέδου','Milestone reward')}</strong><em>${currencyAmount(level*10,'+')}</em></div>${claimed?'<b><i class="ph-bold ph-check" aria-hidden="true"></i></b>':reached?`<button data-action="claim-milestone" data-level="${level}">${tr('Jetzt abholen','Recoger ahora','Πάρε τώρα','Claim now')}</button>`:'<b><i class="ph-bold ph-lock" aria-hidden="true"></i></b>'}</article>`;
 };
 return `<div class="v3-shell page cinematic-subpage cinematic-profile">${cinematicBackground(assets.backgrounds.cinematic.profile)}${top()}<section class="profile-hero-v3"><img src="${assets.characters.tula.poses.profile}" alt="Tula"><h1>${s.profile.name||tr('Inselentdecker','Explorador','Εξερευνητής')}</h1><strong>${tr('Inselstufe','Nivel de isla','Επίπεδο νησιού','Island level')} ${lv} · ${stars} ${tr('Meistersterne','estrellas maestras','αστέρια δεξιοτεχνίας','mastery stars')}</strong><button class="profile-settings-link" data-action="navigate" data-route="child-profile">${tr('Kinderprofil anpassen','Ajustar perfil infantil','Ρύθμιση παιδικού προφίλ')}</button><button class="profile-settings-link" data-action="navigate" data-route="language-select"><i class="ph-bold ph-translate" aria-hidden="true"></i> ${tr('Sprachpaar ändern','Cambiar idiomas','Αλλαγή γλωσσών')}</button></section>${progressCard()}${renderProfileLanguageSection()}<section class="profile-stats"><div><img class="reward-art" src="${assets.rewards.streak}" alt=""><strong>${s.progress.streak}</strong><small>${tr('Lerntage','Días','Ημέρες')}</small></div><div><img class="reward-art" src="${assets.rewards.currencyShell}" alt=""><strong>${s.progress.shells}</strong><small>${tr('Muscheln','Conchas','Κοχύλια')}</small></div><div><img class="reward-art" src="${assets.rewards.xpStar}" alt=""><strong>${stars}</strong><small>${tr('Meistersterne','estrellas maestras','αστέρια δεξιοτεχνίας','mastery stars')}</small></div></section><section class="milestone-list"><div class="section-head"><div><h2>${tr('Deine nächsten Schätze','Tus próximos tesoros','Οι επόμενοι θησαυροί σου','Your next treasures')}</h2></div></div>${visible.map(milestoneCard).join('')}${history.length?`<details class="milestone-history"><summary>${tr('Alle Inselstufen ansehen','Ver todos los niveles de isla','Δες όλα τα επίπεδα νησιού','View all island levels')}</summary>${history.map(milestoneCard).join('')}</details>`:''}</section><button class="secondary" data-action="open-settings"><i class="ph-bold ph-gear" aria-hidden="true"></i> ${tr('Eltern & Einstellungen','Padres y ajustes','Γονείς και ρυθμίσεις')}</button></div>${nav('profile')}`;
}

export function renderSettings({top,nav}){
 const s=getState();
 return `<div class="v3-shell page cinematic-subpage cinematic-profile">${cinematicBackground(assets.backgrounds.cinematic.profile)}${top('profile')}<section class="page-title"><span class="eyebrow">${tr('ELTERNBEREICH','ZONA DE PADRES','ΠΕΡΙΟΧΗ ΓΟΝΕΩΝ')}</span><h1>${tr('Lernumgebung','Entorno de aprendizaje','Περιβάλλον μάθησης')}</h1></section><section class="parent-summary"><div><strong>${levelFromXp(s.progress.xp)}</strong><small>${tr('Inselstufe','Nivel de isla','Επίπεδο νησιού','Island level')}</small></div><div><strong>${s.progress.xp}</strong><small>XP</small></div><div><strong>${Math.min(3,s.progress.daily)}/3</strong><small>${tr('Tagesziel','Meta diaria','Ημερήσιος στόχος')}</small></div></section>${accountMarkup()}<section class="audience-setting"><strong>${tr('Wer nutzt dieses Profil?','¿Quién usa este perfil?','Ποιος χρησιμοποιεί αυτό το προφίλ;')}</strong><p>${tr('Es wird kein Geburtsdatum gespeichert. Kinderprofile erhalten immer die strengste Werbe- und Datenschutzbehandlung.','No se guarda ninguna fecha de nacimiento. Los perfiles infantiles reciben la protección más estricta.','Δεν αποθηκεύεται ημερομηνία γέννησης. Τα παιδικά προφίλ έχουν τη μέγιστη προστασία.')}</p><div><button class="${s.profile.audience!=='adult'?'active':''}" data-action="set-audience" data-audience="child">${tr('Kinderprofil','Perfil infantil','Παιδικό προφίλ')}</button><button class="${s.profile.audience==='adult'?'active':''}" data-action="set-audience" data-audience="adult">${tr('Erwachsener','Adulto','Ενήλικας')}</button></div></section><label class="setting-row"><span><i class="ph-bold ph-speaker-high" aria-hidden="true"></i> <b>${tr('Audio und Sprache','Audio y voz','Ήχος και φωνή')}</b></span><input type="checkbox" data-action="toggle-setting" data-setting="sound" ${s.settings.sound?'checked':''}></label><label class="setting-row"><span><i class="ph-bold ph-sparkle" aria-hidden="true"></i> <b>${tr('Animationen','Animaciones','Κινήσεις')}</b></span><input type="checkbox" data-action="toggle-setting" data-setting="motion" ${s.settings.motion?'checked':''}></label><section class="parent-note"><strong>${tr('Elternhinweis','Nota para padres','Σημείωση για γονείς')}</strong><p>${tr('Lernfortschritt wird lokal und mit Elternkonto zusätzlich in der Cloud gesichert. Die Sprachstufen orientieren sich am GER, sind aber kein offizielles Zertifikat. Echtgeldkäufe verwenden ausschließlich den App Store; Werbung ist freiwillig und nicht personalisiert.','El progreso se guarda localmente y, con cuenta parental, también en la nube. Los niveles se orientan al MCER, pero no son un certificado oficial. Las compras usan solo la tienda y los anuncios son voluntarios.','Η πρόοδος αποθηκεύεται τοπικά και στο cloud με γονικό λογαριασμό. Τα γλωσσικά επίπεδα βασίζονται στο CEFR, αλλά δεν αποτελούν επίσημο πιστοποιητικό. Οι αγορές γίνονται μόνο μέσω του καταστήματος και οι διαφημίσεις είναι προαιρετικές.','Progress is stored locally and, with a parent account, in the cloud. Language levels follow CEFR guidance but are not an official certificate. Purchases use the app store and ads are voluntary.')}</p></section><button class="danger-soft" data-action="reset-progress">${tr('Testfortschritt zurücksetzen','Restablecer progreso','Επαναφορά προόδου')}</button></div>${nav('profile')}`;
}
