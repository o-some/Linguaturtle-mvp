import {
  getState, setState, registerAction, levelFromXp, resetState,
...
function accountMarkup(){
 ...
 if(account.user)return `<section class="account-card"><div class="account-heading"><i class="ph-bold ph-cloud-check"></i><div><small>${tr('ELTERNKONTO','CUENTA PARENTAL','ΓΟΝΙΚΟΣ ΛΟΓΑΡΙΑΣΜΟΣ')}</small><h2>${safe(account.user.email)}</h2></div></div><div class="sync-status sync-${account.syncStatus}"><i class="ph-bold ${account.syncStatus==='synced'?'ph-check-circle':account.syncStatus==='offline'?'ph-wifi-slash':'ph-arrows-clockwise'}"></i><div><strong>${syncLabel(account.syncStatus)}</strong><small>${formatCloudTime(account.lastSyncedAt)}</small></div></div>${account.message?`<p class="account-message">${safe(account.message)}</p>`:''}${account.error?`<p class="account-error">${safe(account.error)}</p>`:''}<button class="primary" data-action="sync-account-now" ${busy?'disabled':''}><i class="ph-bold ph-arrows-clockwise"></i> ${tr('Jetzt synchronisieren','Sincronizar ahora','Συγχρονισμός τώρα')}</button><button class="secondary" data-action="account-sign-out" ${busy?'disabled':''}>${tr('Abmelden','Cerrar sesión','Αποσύνδεση')}</button><button class="account-delete" data-action="account-delete" ${busy?'disabled':''}>${tr('Elternkonto und Cloud-Daten löschen','Eliminar cuenta y datos de la nube','Διαγραφή λογαριασμού και δεδομένων cloud')}</button></section>${conflictMarkup(account)}`;
